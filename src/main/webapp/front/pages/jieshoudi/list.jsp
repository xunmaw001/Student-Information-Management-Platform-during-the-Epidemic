...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">接受地<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../jieshoudi/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.jieshoudiTypes=="" || searchForm.jieshoudiTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in jieshoudiTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.jieshoudiTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../jieshoudi/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加接受地信息
                </button>-->
            </div>
            <table class="layui-table" lay-skin="nob">
                ...
                <tbody>
                    <tr v-for="(item,index) in dataList" v-bind:key="index">
                        <td>{{item.jieshoudiUuidNumber}}</td>
                        <td>{{item.jieshoudiName}}</td>
                        <td>{{item.jieshoudiAddress}}</td>
                        <td>{{item.jieshoudiValue}}</td>
                        <td><button v-if="item.jieshoudiContent!=null&&item.jieshoudiContent!=''" @click="showContentFunction(item.jieshoudiContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        ...
                        -->
                            <button @click="jump('../jieshoudiQianwang/add.jsp?jieshoudiId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                前往此接受地
                            </button>
                            <button v-if="item.jieshoudiTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`jieshoudi/update`, 'post', {
                    id:id,
//                    jieshoudiTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`jieshoudi/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('操作成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/jieshoudi/list.jsp")
        ...
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.jieshoudiTypes = $(this).attr('index');
            pageList();
        });
            ...
            jieshoudiTypesSelect();
                                                   //当前表的 接受地类型 字段 字典表查询方法
           function jieshoudiTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jieshoudi_types", 'get', {}, function (res) {
                   if(res.code == 0){
                       vue.jieshoudiTypesList = res.data.list;
                   }
               });
           }
            ...
            function pageList() {
                // 获取列表数据
                http.request('jieshoudi/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('jieshoudi/list', 'get', vue.searchForm, function (res1) {
