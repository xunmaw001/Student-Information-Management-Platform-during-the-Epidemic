...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">新闻信息<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../news/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.newsTypes=="" || searchForm.newsTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in newsTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.newsTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../news/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加新闻信息信息
                </button>-->
            </div>
            <table class="layui-table" lay-skin="nob">
                ...
                <tbody>
                    <tr v-for="(item,index) in dataList" v-bind:key="index">
                        <td>{{item.newsName}}</td>
                        <td><img :src="baseUrl+item.newsPhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td>{{item.newsValue}}</td>
                        <td>{{item.insertTime}}</td>
                        <td><button v-if="item.newsContent!=null&&item.newsContent!=''" @click="showContentFunction(item.newsContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../news/add.jsp?newsId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.newsTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`news/update`, 'post', {
                    id:id,
//                    newsTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`news/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/news/list2.jsp")

        // var id = http.getParam('id');

        //新闻类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.newsTypes = $(this).attr('index');
            pageList();
        });
           ...
           function newsTypesSelect() {
            ...
            function pageList() {
                // 获取列表数据
                http.request('news/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('news/list', 'get', vue.searchForm, function (res1) {
