...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">学校文件<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../wenjianbiao/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.wenjianbiaoTypes=="" || searchForm.wenjianbiaoTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in wenjianbiaoTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.wenjianbiaoTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../wenjianbiao/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加学校文件信息
                </button>-->
            </div>
            <table class="layui-table" lay-skin="nob">
                ...
                <tbody>
                    <tr v-for="(item,index) in dataList" v-bind:key="index">
                        <td>{{item.wenjianbiaoName}}</td>
                        <td>{{item.wenjianbiaoUuidNumber}}</td>
                        <td><img :src="baseUrl+item.wenjianbiaoPhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td v-if="item.wenjianbiaoFile" @click="jump(baseUrl+item.wenjianbiaoFile)" style="color: red">
                            文件下载
                        </td>
                        <td v-else>
                            无文件
                        ...
                        <td>{{item.wenjianbiaoValue}}</td>
                        <td><button v-if="item.wenjianbiaoContent!=null&&item.wenjianbiaoContent!=''" @click="showContentFunction(item.wenjianbiaoContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.shangxiaValue}}</td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../wenjianbiao/add.jsp?wenjianbiaoId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.wenjianbiaoTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
...
//                    wenjianbiaoTypes:1,
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/wenjianbiao/list2.jsp")
