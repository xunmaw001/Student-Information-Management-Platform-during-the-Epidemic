...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">学校文件收藏<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../wenjianbiaoCollection/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;flex-wrap: wrap;justify-content: space-evenly;">
                <div v-for="(item,index) in dataList" :key="index" style="flex: 0 0 25%;padding: 3px">
                    <div :style='{"padding":"10px","borderColor":"var(--publicSubColor)","boxShadow":"0 0 6px var(--publicSubColor)","margin":"0 0 10px 0","borderColor":"rgba(0,0,0,.3)","backgroundColor":"#fff","borderRadius":"4px","borderWidth":"0","borderStyle":"solid"}'>
                            <i @click="deleteWenjianbiaoCollection(item.id)" class="layui-icon" style="float:right;color:red;margin-top: -10px;font-size:20px">&#xe640;</i>
                    <!-- class="animation-box"-->
                        <img @click="jumpCheck('../wenjianbiao/detail.jsp?id='+item.wenjianbiaoId , item.wenjianbiaoDelete == null?'':item.wenjianbiaoDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)"
                             ...
                             :src="item.wenjianbiaoPhoto?baseUrl+item.wenjianbiaoPhoto.split(',')[0]:''">
                        <div @click="jumpCheck('../wenjianbiao/detail.jsp?id='+item.wenjianbiaoId , item.wenjianbiaoDelete == null?'':item.wenjianbiaoDelete , item.shangxiaTypes == null?'':item.shangxiaTypes)">
                            <div v-if="item.wenjianbiaoNewMoney"
                                ...
                                {{item.wenjianbiaoNewMoney}} RMB
                            </div>
                            <div v-else
                                 :style='{"padding":"0","margin":"10px 0 0 0","backgroundColor":"rgba(0,0,0,0)","color":"red","borderRadius":"0","textAlign":"left","width":"100%","fontSize":"16px"}'>
                                {{item.wenjianbiaoValue}}
                            </div>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            searchForm: {
                ...
                ,wenjianbiaoName: null
                ,wenjianbiaoTypes: null
            },
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`wenjianbiaoCollection/update`, 'post', {
                    id:id,
//                    wenjianbiaoCollectionTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`wenjianbiaoCollection/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
            ...
            deleteWenjianbiaoCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('wenjianbiaoCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/wenjianbiaoCollection/list2.jsp")

        // var id = http.getParam('id');

        //类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.wenjianbiaoCollectionTypes = $(this).attr('index');
            pageList();
        });
           ...
           function wenjianbiaoCollectionTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=wenjianbiao_collection_types", 'get', {}, function (res) {
                   if(res.code == 0){
            ...
            //级联表的 学校文件类型 字段 字典表查询
            wenjianbiaoTypesSelect();
            //级联表的 学校文件类型 字段 字典表查询方法
            ...
            function pageList() {
                // 获取列表数据
                http.request('wenjianbiaoCollection/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('wenjianbiaoCollection/list', 'get', vue.searchForm, function (res1) {
