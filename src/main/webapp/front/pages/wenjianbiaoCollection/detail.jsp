            ...
            <div class="row">
                <div class="col-lg-8">
                    <div class="blog-details-wrapper">
                        <div class="thumb">
                            <img :src="baseUrl+detail.wenjianbiaoCollectionPhoto" alt="image">
                        </div>
                        <div class="blog-details-header">
                            <h3 class="blog-details-title">{{title}}</h3>
                            <ul class="post-meta">
                                <li v-if="detail.wenjianbiaoCollectionTypes">类型：
                                    {{detail.wenjianbiaoCollectionValue}}
                                </li>
                            </ul>
                        ...
                        <!-- <div style="padding: 20px;border: 0px solid #f2f2f2;margin-top: 20px;display: flex">
                                 <div class="num-picker">
                                     <button @click="wenjianbiaoCollectionYuyue()" style="height:auto;" :style='{"padding":"0 10px","boxShadow":"0 0 0px rgba(255,0,0,.3)","margin":"0 5px","borderColor":"rgba(0,0,0,.3)","backgroundColor":"rgba(23, 124, 176, 1)","color":"rgba(255, 255, 255, 1)","borderRadius":"6px","borderWidth":"0","width":"auto","lineHeight":"40px","fontSize":"16px","borderStyle":"solid"}' type="button" class="layui-btn btn-submit">
                                         立即预约
                                     </button>
                                 </div>
                             </div>-->
                ...
                <div class="col-lg-4" v-if="wenjianbiaoCollectionRecommendList">
                    <div class="sidebar">
                        <div class="widget">
                            <h3 class="widget-title">系统推荐</h3>
                            <ul class="small-post-list">
                                <li class="small-post-single" v-for="(item,index) in wenjianbiaoCollectionRecommendList"
                                    v-bind:key="index"
                                    @click="jump('../wenjianbiaoCollection/detail.jsp?id='+item.id)">
                                    <div class="thumb"><img width="65px" :src="baseUrl+item.wenjianbiaoCollectionPhoto"
                                                            alt="image">
                                    </div>
                                    <div class="content">
                                        <a class="main_color" class="date">{{item.wenjianbiaoCollectionName}}</a>
                                        <h6 v-if="item.wenjianbiaoCollectionNewMoney" class="post-title"><a
                                                style="color: red">{{item.wenjianbiaoCollectionNewMoney}}</a>RMB</h6>
                                    </div>
                                </li>
                            </ul>
                        </div>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            data: {
                ...
                //系统推荐
                wenjianbiaoCollectionRecommendList: [],
                    // 当前详情页表
                detailTable: 'wenjianbiaoCollection',
            },
            methods: {
                //分享
                share(){
                    let element = createElement("http://localhost:8080/yqxueshengxinxi/front/index.jsp?url="+"./pages/wenjianbiaoCollection/detail.jsp?id="+this.id);
                    element.select();
                ...
                //预约
                wenjianbiaoCollectionYuyue(){
                    let _this = this;
                    var mymessage = confirm("确定要预约吗？");if(!mymessage){return false;}
                    // localStorage.setItem('wenjianbiaoCollectionId', _this.detail.id);
                    // _this.jump("../wenjianbiaoCollectionOrder/add.jsp");

                    let data={
                        wenjianbiaoCollectionId:_this.detail.id,
                        xueshengId:localStorage.getItem("userid"),
                        wenjianbiaoCollectionOrderYesnoTypes:1,
                    }
                    // 提交数据
                    layui.http.requestJson('wenjianbiaoCollectionOrder/add', 'post', data, function (res) {
                        if(res.code == 0) {
                            layui.layer.msg('预约成功', {
                                time: 2000,
                                icon: 6
                            }, function () {
                                _this.jump("../wenjianbiaoCollectionOrder/list.jsp");
                            });
                        }else{
        ...
        layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'util'], function () {
            ...
            localStorage.setItem("goUtl","./pages/wenjianbiaoCollection/detail.jsp?id="+ http.getParam('id'))
            ...
            http.request(`${vue.detailTable}/detail/` + id, 'get', {}, function (res) {
                // 详情信息
                vue.detail = res.data;
                vue.title = vue.detail.wenjianbiaoCollectionName;
                // 轮播图片
                vue.swiperList = vue.detail.wenjianbiaoCollectionPhoto ? vue.detail.wenjianbiaoCollectionPhoto.split(",") : [];
                // 轮播图
                vue.$nextTick(() => {
                    carousel.render({
                        elem: '#swiper',
            ...
            http.request(`wenjianbiaoCollection/list`, 'get', {
                page: 1,
                limit: 5,
                wenjianbiaoCollectionTypes: vue.detail.wenjianbiaoCollectionTypes,
            }, function (res) {
                vue.wenjianbiaoCollectionRecommendList = res.data.list;
            });
