...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">接受地前往<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../jieshoudiQianwang/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.jieshoudiQianwangTypes=="" || searchForm.jieshoudiQianwangTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in jieshoudiQianwangTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.jieshoudiQianwangTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../jieshoudiQianwang/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加接受地前往信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        <td>{{item.jieshoudiName}}</td>
                        <td>{{item.jieshoudiValue}}</td>
                        <td>{{item.jieshoudiQianwangUuidNumber}}</td>
                        <td v-if="item.jieshoudiQianwangFile" @click="jump(baseUrl+item.jieshoudiQianwangFile)" style="color: red">
                            附件下载
                        </td>
                        <td v-else>
                            无附件
                        </td>
                        <td>{{item.jieshoudiQianwangValue}}</td>
                        <td>{{item.qianwangTime}}</td>
                        <td>{{item.didaTime}}</td>
                        <td><button v-if="item.jieshoudiQianwangContent!=null&&item.jieshoudiQianwangContent!=''" @click="showContentFunction(item.jieshoudiQianwangContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../jieshoudiQianwang/add.jsp?jieshoudiQianwangId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.jieshoudiQianwangTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            searchForm: {
                ...
                ,xueshengId: localStorage.getItem('userid')//只能查询自己
                    ,jieshoudiQianwangUuidNumber: ""
                ,jieshoudiQianwangFile: ""
                ,jieshoudiQianwangTypes: ""
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`jieshoudiQianwang/update`, 'post', {
                    id:id,
//                    jieshoudiQianwangTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`jieshoudiQianwang/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/jieshoudiQianwang/list2.jsp")

        // var id = http.getParam('id');

        //前往类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.jieshoudiQianwangTypes = $(this).attr('index');
            pageList();
        });
           ...
           //当前表的 前往类型 字段 字典表查询方法
           function jieshoudiQianwangTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jieshoudi_qianwang_types", 'get', {}, function (res) {
                   if(res.code == 0){
            ...
            function pageList() {
                // 获取列表数据
                http.request('jieshoudiQianwang/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('jieshoudiQianwang/list', 'get', vue.searchForm, function (res1) {
