...
<body>
    ...
    <div id="app">
        ...
        <div class="news-container">
            <h1 class="title">{{detail.tongzhiName}}</h1>
            <div class="auth-container">
                时间：{{detail.insertTime}}
            </div>
            <div style="width: 980px;margin: 0 auto">
                <img width="100%" :src="baseUrl+detail.tongzhiPhoto">
            </div>
            <div class="content" style="word-break: break-all;" v-html="myFilters(detail.tongzhiContent)">
            </div>
    ...
    <script>
        ...
        var vue = new Vue({
            ...
            methods: {
                //分享
                share(){
                    let element = createElement("http://localhost:8080/yqxueshengxinxi/front/index.jsp?url="+"./pages/tongzhi/detail.jsp?id="+this.id);
                    element.select();
                ...
                tongzhiYuyue(){
                    ...
                    // localStorage.setItem('tongzhiId', _this.detail.id);
                    // _this.jump("../tongzhiOrder/add.jsp");

                    let data={
                        tongzhiId:_this.detail.id,
                        xueshengId:localStorage.getItem("userid"),
                        tongzhiOrderYesnoTypes:1,
                    }
                    // 提交数据
                    layui.http.requestJson('tongzhiOrder/add', 'post', data, function (res) {
                        if(res.code == 0) {
                            layui.layer.msg('预约成功', {
                                time: 2000,
                                icon: 6
                            }, function () {
                                _this.jump("../tongzhiOrder/list.jsp");
                            });
                        }else{
        ...
        layui.use(['layer', 'form', 'element', 'carousel', 'http', 'jquery', 'laypage', 'util'], function () {
            ...
            localStorage.setItem("goUtl","./pages/tongzhi/detail.jsp?id="+ http.getParam('id'))
            ...
            vue.detail.id = id;
            // 详情信息
            http.request('tongzhi/detail/' + id, 'get', {}, function(res) {
                vue.detail = res.data;
                vue.title = vue.detail.tongzhiName;
                    vue.detail.tongzhiContent = vue.detail.tongzhiContent.replaceAll("src=\"upload/","src=\""+vue.baseUrl+"upload/").replaceAll(/<img/g,'<img style="width: 100%;"');
            });
        });
    </script>
</body>
