...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">学生<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../xuesheng/list.jsp'?'on':''" @click="jump(item.url)">
...
<!--                <button @click="jump('../xuesheng/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加学生信息
                </button>-->
            </div>
            <table class="layui-table" lay-skin="nob">
                ...
                <tbody>
                    <tr v-for="(item,index) in dataList" v-bind:key="index">
                        <td>{{item.username}}</td>
                        <td>{{item.xueshengName}}</td>
                        <td>{{item.xueshengPhone}}</td>
                        <td>{{item.xueshengIdNumber}}</td>
                        <td><img :src="baseUrl+item.xueshengPhoto" style="width: 100px;height: 100px;object-fit: cover;"></td>
                        <td>{{item.sexValue}}</td>
                        <td>{{item.xueyuanValue}}</td>
                        <td>{{item.banjiValue}}</td>
                        <td>{{item.xueshengEmail}}</td>
                        <td>{{item.jinyongValue}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../xuesheng/add.jsp?xueshengId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.xueshengTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`xuesheng/update`, 'post', {
                    id:id,
//                    xueshengTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`xuesheng/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/xuesheng/list2.jsp")
        ...
        // var id = http.getParam('id');
           ...
           function jinyongTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=jinyong_types", 'get', {}, function (res) {
                   if(res.code == 0){
            ...
            function pageList() {
                // 获取列表数据
                http.request('xuesheng/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('xuesheng/list', 'get', vue.searchForm, function (res1) {
