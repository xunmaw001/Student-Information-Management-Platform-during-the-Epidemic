...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">每日记录<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../xueshengJilu/list.jsp'?'on':''" @click="jump(item.url)">
...
<!--                <button @click="jump('../xueshengJilu/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加每日记录信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        <td>{{item.xueshengJiluUuidNumber}}</td>
                        <td>{{item.xueshengJiluName}}</td>
                        <td v-if="item.xueshengJiluFile" @click="jump(baseUrl+item.xueshengJiluFile)" style="color: red">
                            记录附件下载
                        </td>
                        <td v-else>
                            无记录附件
                        </td>
                        <td>{{item.xueshengJiluTime}}</td>
                        <td><button v-if="item.xueshengJiluContent!=null&&item.xueshengJiluContent!=''" @click="showContentFunction(item.xueshengJiluContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.insertTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../xueshengJilu/add.jsp?xueshengJiluId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.xueshengJiluTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`xueshengJilu/update`, 'post', {
                    id:id,
//                    xueshengJiluTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`xueshengJilu/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/xueshengJilu/list2.jsp")
        ...
        // var id = http.getParam('id');
            ...
            function pageList() {
                // 获取列表数据
                http.request('xueshengJilu/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('xueshengJilu/list', 'get', vue.searchForm, function (res1) {
