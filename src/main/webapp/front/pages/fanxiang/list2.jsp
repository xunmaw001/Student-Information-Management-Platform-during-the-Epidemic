...
<div class="index-sec1">
    <div class="sec1-left fl">
        ...
        <p class="p2">返校申请<b></b></p>
        <ul>
            <li v-for="(item,index) in centerMenu" v-bind:key="index" :class="item.url=='../fanxiang/list.jsp'?'on':''" @click="jump(item.url)">
            ...
            <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;justify-content: space-between">
                <div style="display: flex;height: 60px;align-items: center;margin-left: 15px;">
                    <div class="thisTableType-search" style="margin-left: 15px;"
                         :style='searchForm.fanxiangTypes=="" || searchForm.fanxiangTypes==null?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        全部
                    </div>
                    <div style="margin-left: 15px;" v-for="item in fanxiangTypesList" :index="item.codeIndex" class="thisTableType-search"
                         :style='searchForm.fanxiangTypes==item.codeIndex?{"color":"red","borderBottom":"1px solid red"}:{"color":"#000"}'>
                        {{item.indexName}}
                    </div>
                </div>
<!--                <button @click="jump('../fanxiang/add.jsp')" class="layui-btn layui-btn-lg btn-theme sub_backgroundColor" :style='{"padding":"0 15px","boxShadow":"0 0 8px rgba(0,0,0,0)","margin":"0 0 0 10px","borderColor":"#409EFF","color":"#333","borderRadius":"4px","borderWidth":"0","width":"auto","fontSize":"14px","borderStyle":"solid","height":"40px"}'>
                    <i v-if="true" class="layui-icon">&#xe654;</i> 添加返校申请信息
                </button>-->
            </div>
        ...
        <!-- 级联表 -->
                        <td>{{item.fanxiangUuidNumber}}</td>
                        <td>{{item.fanxiangName}}</td>
                        <td>{{item.fanxiangAddress}}</td>
                        <td v-if="item.fanxiangFile" @click="jump(baseUrl+item.fanxiangFile)" style="color: red">
                            附件下载
                        </td>
                        <td v-else>
                            无附件
                        </td>
                        <td>{{item.fanxiangValue}}</td>
                        <td>{{item.fanxiangFangshiValue}}</td>
                        <td>{{item.fanxiangLujing}}</td>
                        <td>{{item.daoxiaoTime}}</td>
                        <td><button v-if="item.fanxiangContent!=null&&item.fanxiangContent!=''" @click="showContentFunction(item.fanxiangContent)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.insertTime}}</td>
                        <td>{{item.fanxiangYesnoValue}}</td>
                        <td><button v-if="item.fanxiangYesnoText!=null&&item.fanxiangYesnoText!=''" @click="showContentFunction(item.fanxiangYesnoText)" type="button" class="layui-btn layui-btn-success">点击查看</button></td>
                        <td>{{item.fanxiangShenheTime}}</td>
                        <td style="width: 100px;">
                        <!--
                            <button @click="jump('../fanxiang/add.jsp?fanxiangId='+item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                 修改
                            </button>
                        -->
                            ...
                            <button v-if="item.fanxiangTypes==1 && false" @click="wuyong(item.id)" type="button" class="layui-btn layui-btn-sm layui-btn-radius layui-btn-warm">
                                无用按钮
                            </button>
                        </td>
                    </tr>
    ...
    var vue = new Vue({
        ...
        methods: {
            ...
            wuyong(id) {
                ...
                layui.http.requestJson(`fanxiang/update`, 'post', {
                    id:id,
//                    fanxiangTypes:1,
                }, function (res) {
                    if(res.code == 0){
            ...
            deleteData(data){
                ...
                // 删除信息
                layui.http.requestJson(`fanxiang/delete`, 'post', [data.id], (res) => {
                    if(res.code==0){
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery', 'laydate', 'tinymce'], function() {
        ...
        localStorage.setItem("goUtl","./pages/fanxiang/list2.jsp")

        // var id = http.getParam('id');

        //返校申请类型的动态搜素
        $(document).on("click", ".thisTableType-search", function (e) {
            vue.searchForm.fanxiangTypes = $(this).attr('index');
            pageList();
        });


            //当前表的 返校申请类型 字段 字典表查询
            fanxiangTypesSelect();
            //当前表的 返校方式 字段 字典表查询
            fanxiangFangshiTypesSelect();
            //当前表的 申请状态 字段 字典表查询
            fanxiangYesnoTypesSelect();
           //当前表的 返校申请类型 字段 字典表查询方法
           function fanxiangTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fanxiang_types", 'get', {}, function (res) {
                   if(res.code == 0){
                       vue.fanxiangTypesList = res.data.list;
                   }
               });
           }
           //当前表的 返校方式 字段 字典表查询方法
           function fanxiangFangshiTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fanxiang_fangshi_types", 'get', {}, function (res) {
                   if(res.code == 0){
           ...
           function fanxiangYesnoTypesSelect() {
               http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=fanxiang_yesno_types", 'get', {}, function (res) {
                   if(res.code == 0){
            ...
            function pageList() {
                // 获取列表数据
                http.request('fanxiang/list', 'get', vue.searchForm, function (res) {
                    ...
                    laypage.render({
                        ...
                        jump: function (obj, first) {
                            ...
                            //首次不执行
                            if (!first) {
                                http.request('fanxiang/list', 'get', vue.searchForm, function (res1) {
