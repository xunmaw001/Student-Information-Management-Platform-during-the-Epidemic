...
<!--/Pre Loader -->
                        ...
                        <div class="widget-area-2 lochana-box-shadow">
                            ...
                            <div class="table-responsive mb-3" id="tableDiv">
                                <div class="col-sm-12">
                                        ...
                                        <label>账户状态</label>
                                        <select name="jinyongTypesSelectSearch" id="jinyongTypesSelectSearch" aria-controls="tableId" class="form-control form-control-sm"></select>
                                        ...
                                        <span class="导入导出">
                                            导入<input name="file" id="xueshengExcelFile" type="file" class="btn btn-success" style="width: 70px!important; line-height: 35px;">
                                            <button class="btn btn-success" onclick="download('/upload/xueshengMuBan.xls')">导入模板下载</button>
                                        </span>
                                ...
                                <table id="tableId" class="table table-bordered table-striped">
                                    ...
                                    <tr>
                                        ...
                                        <th onclick="sort('xuesheng_name')">学生姓名<i id="xueshengNameIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('xuesheng_phone')">学生手机号<i id="xueshengPhoneIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('xuesheng_id_number')">学生身份证号<i id="xueshengIdNumberIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('xuesheng_photo')">学生头像<i id="xueshengPhotoIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('sex_types')">性别<i id="sexTypesIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('xueyuan_types')">学院<i id="xueyuanTypesIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('banji_types')">班级<i id="banjiTypesIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('xuesheng_email')">学生邮箱<i id="xueshengEmailIcon" class="fa fa-sort"></i></th>
                                        <th onclick="sort('jinyong_types')">账户状态<i id="jinyongTypesIcon" class="fa fa-sort"></i></th>
                                        <th>操作</th>
                                    </tr>
                                    </thead>
                                    <tbody id="thisTbody">
    ...
    <!-- 本表的查询条件 -->
        ...
        function setDataRow(item, number) {
            ...
            //账户状态
            var jinyongTypesCell = document.createElement('td');
            jinyongTypesCell.innerHTML = item.jinyongValue;
            row.appendChild(jinyongTypesCell);
            ...
            //禁用启用账户
            var jinqiyongBtn = document.createElement('button');
            var jinqiyongAttr = 'jinqiyong(' + item.id + ','+item.jinyongTypes+')';
            jinqiyongBtn.setAttribute("type", "button");
            jinqiyongBtn.setAttribute("class", "btn btn-warning btn-sm 修改");
            jinqiyongBtn.setAttribute("onclick", jinqiyongAttr);
            if(item.jinyongTypes==1){
                jinqiyongBtn.innerHTML = "禁用账户";
            }else{
                jinqiyongBtn.innerHTML = "启用账户";

            }
            btnGroup.appendChild(jinqiyongBtn);
            //重置密码
        ...
        function upload() {
            /*导入数据*/
            $('#xueshengExcelFile').fileupload({
                url: baseUrl + 'file/upload',
                headers: {token: window.sessionStorage.getItem("token")},
                dataType: "json",
                done: function (e, data) {
                    http("xuesheng/batchInsert?fileName="+data.result.file, "get", {} , (res) => {
                        if(res.code == 0){
        ...
        function jinqiyong(id,jinyongTypes) {
            let msg="";
            if(jinyongTypes==1){
            ...
            if (confirm(msg) == true) {
                httpJson("xuesheng/update", "POST", {id:id,jinyongTypes:jinyongTypes,}, (res) => {
                    if(res.code == 0){
                        getDataList();layui.layer.msg("操作成功", {time: 2000,icon: 6});
                    }
                });
            }
        }
